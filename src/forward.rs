pub(crate) mod internal {
    ...
    pub fn forward(p: &Parameters, qs: &JointState) -> na::IsometryMatrix3<f32> {
        let q: JointState = [
            qs[0] * p.sign_corrections[0] as f32 - p.offsets[0],
            qs[1] * p.sign_corrections[1] as f32 - p.offsets[1],
            qs[2] * p.sign_corrections[2] as f32 - p.offsets[2],
            qs[3] * p.sign_corrections[3] as f32 - p.offsets[3],
            qs[4] * p.sign_corrections[4] as f32 - p.offsets[4],
            qs[5] * p.sign_corrections[5] as f32 - p.offsets[5],
        ];
